...
import com.paulniu.panllibrary.imchat.adapter.IMChatKeyboardSmilyViewPagerAdapter;
...
public class IMChatKeyboardSmilyFragment extends BaseIMFragment {
    ...
    public void initViewFindViewById(View root) {
        if (root == null) return;
        try {
            vpfsEmojiContanier = root.findViewById(R.id.viewpagerEmojiContanier);
            llFsDot = root.findViewById(R.id.llDot);
        } catch (Exception ex) {
        }
    }
