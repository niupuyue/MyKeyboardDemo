...
public class IMChatKeyboardRecorderFragment extends BaseIMFragment implements
    ...
    private Runnable recordImageRefresh = new Runnable() {
        public void run() {
            ...
            try {
                ...
                setHoldToSpeakImage();
                handler.postDelayed(recordImageFakeRefresh, 150L);
            } catch (Exception ex) {
            }
        }
    };
    ...
    private void setHoldToSpeakImage() {
        if (this.amplitude > VOICE_NUM_MAX) {
            this.amplitude = VOICE_NUM_MAX;
        }
        OnSendVoiceEvent event = new OnSendVoiceEvent();
        if (this.amplitude >= 0 && this.amplitude < VOICE_NUM_LEVEL_1) {
            event.status = OnSendVoiceEvent.TYPE_CHANGE_VOICE;
            event.showImageType = OnSendVoiceEvent.SHOW_IMAGE_TYPE_1;
        } else if (this.amplitude >= VOICE_NUM_LEVEL_1 && this.amplitude < VOICE_NUM_LEVEL_2) {
            event.status = OnSendVoiceEvent.TYPE_CHANGE_VOICE;
            event.showImageType = OnSendVoiceEvent.SHOW_IMAGE_TYPE_2;
        } else if (this.amplitude >= VOICE_NUM_LEVEL_2 && this.amplitude < VOICE_NUM_LEVEL_3) {
            event.status = OnSendVoiceEvent.TYPE_CHANGE_VOICE;
            event.showImageType = OnSendVoiceEvent.SHOW_IMAGE_TYPE_3;
        } else if (this.amplitude >= VOICE_NUM_LEVEL_3 && this.amplitude < VOICE_NUM_LEVEL_4) {
            event.status = OnSendVoiceEvent.TYPE_CHANGE_VOICE;
            event.showImageType = OnSendVoiceEvent.SHOW_IMAGE_TYPE_4;
        } else if (this.amplitude >= VOICE_NUM_LEVEL_4) {
            event.status = OnSendVoiceEvent.TYPE_CHANGE_VOICE;
    ...
    public void initViewFindViewById(View root) {
        try {
            ivRfVoice = root.findViewById(R.id.ivVoice);
            tvRfVoice = root.findViewById(R.id.tvVoice);
        } catch (Exception ex) {
        }
    }
    ...
    public void initViewListener() {
        ivRfVoice.setOnTouchListener(this);
    }
    ...
    private void initRecord() {
        try {
            if (ChatRecorderKit == null) {
                ChatRecorderKit = new RecorderKit(iWxCallback, MAX_RECORD_TIME, MIN_RECORD_TIME, PERIOD_RECORD_TIME);
            }
        } catch (Exception ex) {
        }
    }
    ...
    private void createAudioMessage(String audioPath, int duration) {
        try {
            ...
            mListener.sendVoiceMessage(null);
        } catch (Exception ex) {
        }
    }
    ...
    private void startRecordOperation() {
        ...
        try {
            ...
            // 开始执行倒计时
            mTimer.schedule(timerTasks, 0, 1000L);
            isSending = true;
        } catch (Exception ex) {
        }
    }
    ...
    private void moveRecord(MotionEvent event) {
        ...
        if (!isSending) return;
        try {
            if (isMoveToCancel && (int) Math.abs((event.getY() - startY)) > 200) {
                cancelRecord();
