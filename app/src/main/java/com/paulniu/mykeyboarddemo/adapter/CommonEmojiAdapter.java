 ...
 * Version:
 */
public abstract class CommonEmojiAdapter<T> extends RecyclerView.Adapter<EmojiViewHolder> {

    private static final String TAG = CommonEmojiAdapter.class.getSimpleName();
    ...
    private DiffUtil.Callback callback = new DiffUtil.Callback() {
        ...
        public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
            return CommonEmojiAdapter.this.areItemsTheSame(oldItemPosition, newItemPosition);
        }

        @Override
        public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
            return CommonEmojiAdapter.this.areContentsTheSame(oldItemPosition, newItemPosition);
        }

        @NonNull
        @Override
        public Object getChangePayload(int oldItemPosition, int newItemPosition) {
            return CommonEmojiAdapter.this.getChangePayload(oldItemPosition, newItemPosition);
        }
    };
    ...
    private EmojiViewHolder.OnClickListener clickListener;

    private EmojiViewHolder.OnLongClickListener longClickListener;

    private CommonEmojiAdapter(Context context, List<T> dataList) {
        this.layoutInflater = LayoutInflater.from(context);
        this.dataList = clone(dataList);
        this.newDataList = new ArrayList<>();
    }

    protected CommonEmojiAdapter(Context context, List<T> dataList, int layoutId) {
        this(context, dataList);
        this.layoutId = layoutId;
    }

    protected CommonEmojiAdapter(Context context, List<T> dataList, MultiTypeSupport<T> multiTypeSupport) {
        this(context, dataList);
...
//        }).start();
    ...
    public EmojiViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        return new EmojiViewHolder(layoutInflater.inflate(layoutId, parent, false));
    }

    @Override
    public void onBindViewHolder(EmojiViewHolder holder, int position, List<Object> payloads) {
        if (payloads.isEmpty()) {
    ...
    public void setClickListener(EmojiViewHolder.OnClickListener clickListener) {
        this.clickListener = clickListener;
    }

    public void setLongClickListener(EmojiViewHolder.OnLongClickListener longClickListener) {
     ...
     */
    protected abstract void partialBindData(EmojiViewHolder holder, @NonNull Bundle bundle);
     ...
     * @param data   Data
     */
    protected abstract void entirelyBindData(EmojiViewHolder holder, T data, int position);
